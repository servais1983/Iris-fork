...
class VtHandler(object):
    ...
    def _validate_report(self, report):
        ...
        if results.get('response_code') == 0:
            self.log.info(f'VT has no data for this IOC :: {results.get("verbose_msg")}')
            return InterfaceStatus.I2Success(data={'no_data': True, 'verbose_msg': results.get('verbose_msg')})
    ...
    def handle_vt_domain(self, ioc):
        ...
        if report.get('no_data'):
            self.log.info('No VT data for this domain - tagging as vt:no_data')
            if ioc.ioc_tags is None:
                ioc.ioc_tags = ""
            if 'vt:no_data' not in ioc.ioc_tags.split(','):
                ioc.ioc_tags = f"{ioc.ioc_tags},vt:no_data".lstrip(',')
            return InterfaceStatus.I2Success("VT has no data for this domain")
    ...
    def handle_vt_ip(self, ioc):
        ...
        report = status.get_data()
        if report.get('no_data'):
            self.log.info('No VT data for this IP - tagging as vt:no_data')
            if ioc.ioc_tags is None:
                ioc.ioc_tags = ""
            if 'vt:no_data' not in ioc.ioc_tags.split(','):
                ioc.ioc_tags = f"{ioc.ioc_tags},vt:no_data".lstrip(',')
            return InterfaceStatus.I2Success("VT has no data for this IP")
    ...
    def handle_vt_hash(self, ioc):
        ...
        report = status.get_data()
        if report.get('no_data'):
            self.log.info('No VT data for this hash - tagging as vt:no_data')
            if ioc.ioc_tags is None:
                ioc.ioc_tags = ""
            if 'vt:no_data' not in ioc.ioc_tags.split(','):
                ioc.ioc_tags = f"{ioc.ioc_tags},vt:no_data".lstrip(',')
            return InterfaceStatus.I2Success("VT has no data for this hash")
